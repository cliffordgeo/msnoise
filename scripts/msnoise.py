...
@cli.command(name='compute_mwcs_syn')
@click.pass_context
def compute_mwcs_syn(ctx):
    """Computes the MWCS synthetics jobs"""
    from ..s05compute_mwcs_syn import main
    threads = ctx.obj['MSNOISE_threads']
    delay = ctx.obj['MSNOISE_threadsdelay']
    loglevel = ctx.obj['MSNOISE_verbosity']
    if threads == 1:
...
@cli.command(name='compute_dtt_syn')
@click.pass_context
@click.option('-i', '--interval', default=1.0, help='Number of days before now to\
 search for modified Jobs')
def compute_dtt_syn (ctx, interval):
    """Computes the dt/t jobs based on the new MWCS data"""
    from ..s06compute_dtt_syn import main
    threads = ctx.obj['MSNOISE_threads']
    delay = ctx.obj['MSNOISE_threadsdelay']
    loglevel = ctx.obj['MSNOISE_verbosity']
    if threads == 1:
...
@click.option('-o', '--outfile', help='Output filename (?=auto)',
              default=None, type=str)
@click.pass_context
def dvvsyn(ctx, mov_stack, comp, dttname, filterid, pair, all, show, outfile):
    """Plots the dv/v (parses the dt/t results)\n
    Individual pairs can be plotted extra using the -p flag one or more times.\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI -p ID_KWUI_ID_TRWI\n
    ...
    """
    if ctx.obj['MSNOISE_custom']:
        from dvvsyn import main
    else:
        from ..plots.dvvsyn import main
    main(mov_stack, dttname, comp, filterid, pair, all, show, outfile)
    
'''added by tom 6/22 - requires hypen instead of underscore to call for some reason?'''
@plot.command()
@click.option('-f', '--filterid', default=1, help='Filter ID')
@click.option('-c', '--comp', default="ZZ", help='Components (ZZ, ZR,...)')
@click.option('-m', '--mov_stack', default=0, help='Plot specific mov stacks')
...
@click.option('-o', '--outfile', help='Output filename (?=auto)',
              default=None, type=str)
@click.pass_context
def dvv_cust(ctx, mov_stack, comp, dttname, filterid, pair, all, show, outfile):
    """Plots the dv/v (parses the dt/t results)\n
    Individual pairs can be plotted extra using the -p flag one or more times.\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI -p ID_KWUI_ID_TRWI\n
    ...
    """
    if ctx.obj['MSNOISE_custom']:
        from dvv_cust import main
    else:
        from ..plots.dvv_cust import main
    main(mov_stack, dttname, comp, filterid, pair, all, show, outfile)
...
@click.option('-o', '--outfile', help='Output filename (?=auto)',
              default=None, type=str)
# @click.option('-t', '--test', help='Just a test', is_flag=True)
@click.option('-t', '--pair_type', default=None, help='Default all pairs, AConly, greeley, nongreeley, all, allAC')


@click.pass_context
def dvvGreeley(ctx, mov_stack, comp, dttname, filterid, pair, all, show, outfile, pair_type):
    """Plots the dv/v (parses the dt/t results)\n
    Individual pairs can be plotted extra using the -p flag one or more times.\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI -p ID_KWUI_ID_TRWI\n
    ...
    """
    if ctx.obj['MSNOISE_custom']:
        from dvvGreeley import main
    else:
        from ..plots.dvvGreeley import main
    main(mov_stack, dttname, comp, filterid, pair, all, show, outfile, pair_type)
...
@click.argument('extra_args', nargs=-1, type=click.UNPROCESSED, callback=parse_extra_args)
@click.pass_context
def distance_syn(ctx, filterid, comp, ampli, show, outfile, refilter,
             virtual_source, extra_args):
    """Plots the REFs of all pairs vs distance for synthetics"""
    
        
        
    if ctx.obj['MSNOISE_custom']:
        from distance_syn import main
    else:
        from ..plots.distance_syn import main
    main(filterid, comp, ampli, show, outfile, refilter, virtual_source,
         **extra_args)

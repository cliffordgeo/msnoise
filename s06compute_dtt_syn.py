                ...
                for mov_stack in mov_stacks:
                    ...
                    for job in jobs:
                        ...
                        if os.path.isfile(day):
                            ...
                            '''modifying dtt_min and dtt_width based on Ugalde 2014 - Tom, 9/27/20'''
                            #dist already defined a
                            # srf = dist / 3.5 #interstation distance / surface wave speed
                            # lmlag = -srf + -2*f.mwcs_wlen
                            # rmlag = srf + 2*f.mwcs_wlen
                            # lMlag = -srf + -5*f.mwcs_wlen
                            # rMlag = srf + 5*f.mwcs_wlen
                            
                            '''synthetic parameters - 10/11'''
                            # #need to add 1km to each distance
                            dist = dist+1
                            # #set bounds between 3 and 1 km/s
                            # three = dist/3
                            # one = dist/1
                            # lmlag = -three
                            # rmlag = three
                            # lMlag = -one
                            # rMlag = one
                            #setting to 3km/s because CCF waveforms are so short
                            lmlag = -dist / 3
                            rmlag = dist / 3
                            lMlag = -dist/ 1
                            rMlag = dist/ 1
                            ...
                            if first:
                                ...
                                # print(df['err'])
                                # errArray = df['err']
                                # errArray = df['err']            #synthetic mod - setting error to 0.1 dt
                                errArray=(df.dt)*0.1
                                cohArray = df['coh']
                                pairArray = [pair, ]
                                first = False
                                ...
                                errArray = np.vstack((errArray, errArray))
...
#                             '''I think above is where ALL is defined and below is where custom groups could be implemented - Tom'''
...
#                                     net1, sta1, net2, sta2 = pair.split('_')
#                                     # if sta1 in groups[group] and \
#                                     #                 sta2 in groups[group]:    #change these lines so both stations must be in listed pair
                                    
#                                     # st1 = 'XU_GRCO'
#                                     # st2 = 'XU_GRRO'
                                    
#                                     # st1match = [s for s in groups['greeley']  if sta1 in s]
#                                     # st2match = [s for s in st1match if sta2 in s]
                                    
#                                     # if len(st2match) == 1:  
#                                         # print(st2match)
                                    
#                                     if pair in groups[group]:
#                                         pairindex.append(j)
#                                 pairindex = np.array(pairindex)
